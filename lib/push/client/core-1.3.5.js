...
perfmjs.plugin('includeRes', function($$) {
	...
	$$.includeRes = {
		...
		loadModules: function(options) {
			//var loadModules_execstart = $$.utils.now();
			options = $$.utils.extend({name:'js-comm', type:'js', mdCallback:function(){}, handleUrlsCallback:function(){}, afterLoadedCallback:function(){}}, options);
			if (typeof $$.includeRes.sources != 'undefined') {
				...
				for (var i = 0; i < modLen; i++) {
					...
					if ($$.includeRes.loadedModuleName[sources[i]['n']]) {
						continue;
					}
		...
		_parseLoadedRes: function(options) {
			...
			options = $$.utils.extend({loadfile:'core-[0-9]+.[0-9]+.[0-9]+.js|core-[0-9]+.[0-9]+.[0-9]+.min.js|onlyforload.js|core.min.js|core.js', isScript:false, src:''}, options);
