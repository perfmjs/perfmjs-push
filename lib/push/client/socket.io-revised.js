...
Socket.prototype.onClose = function (reason, desc) {
  if ('opening' == this.readyState || 'open' == this.readyState) {
    ...
    //FIXME by tony at 2014/6/18, 解决服务器关闭后的IE6内存溢出问题,下面的setTimeout可以不注释 begin
    setTimeout(function() {
      self.writeBuffer = [];
      self.callbackBuffer = [];
...
Polling.prototype.doClose = function(){
  ...
  if ('open' == this.readyState) {
    ...
    //FIXME by tony at 2014/6/18, 解决服务器关闭后的IE6内存溢出问题 begin
    this.readyState = 'closed';
    //FIXME by tony at 2014/6/8, 解决服务器关闭后的IE6内存溢出问题 end
    close();
  } else {
