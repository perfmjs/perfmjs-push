...
perfmjs.plugin('pushServer', function($$) {
    $$.base("pushServer", {
                ...
                try {
                    self.option('redisSub').subscribe(self.getPublishKey(room), function(err, reply, redis) {
                        ...
                        redis.on("message", function (channel, message) {
                            $$.logger.info('refresh data comming!');
        ...
        getStoredLastDataKey: function(room) {
            return this.option('redisChannelPrefix') + "lastData/" + room;
        },
        /**
        ...
        getRemoteData: function(remoteDataUrl, callback) {
            ...
            try {
                (new (require("shred"))()).get({
                    url: remoteDataUrl,
                    headers: {Accept: "application/json"}, timeout: {minutes: 0, seconds: 20}, //time out in 0 minute and 30 seconds
                    on: {
                        response: function (response) {
                            callback(response.content.body);
                        }, timeout: function (request) {
                            $$.logger.info('请求Timeout,3秒后重新请求, remoteDataUrl=' + remoteDataUrl);
                            setTimeout(function () {
                ...
                $$.logger.error('error on getRemoteData: ' + err.message);
            }
        },
        /**
         * 根据业务情况，子类需要重写该业务方法
