    ...
    $$.base("pushServer", {
        ...
        startup: function(options) {
            ...
            $$.utils.forEach(this.option('rooms'), function(room, index) {
                var profiler = $$.profiler.newInstance().start(room, ($$.sysConfig.config.get('profile')||{})['roomConnCount'] || 100, 'roomConnCount'),
                    chat = server.of("/" + room).on('connection', function (socket) {
                    profiler.stop(room, chat.sockets.length, 'pushServer#startup同时在线的长连接数过多!', 'roomConnCount');
                    //TODO 权限校验，只允许同源域名或IP白名单访问该连接
                    socket.on('message', function(data) {
                        self._onMessage(socket, data, room);
                    });
                    ...
                    var originalPacketFn = socket.packet;
                    socket.packet = function(data) {
                        if (self._shouldDoPacket(data, room)) {
                            originalPacketFn.apply(this, data);
                        }
                    };
                ...
                try {
                    self.option('redisSub').subscribe(self.getPublishKey(room), function(err, reply, redis) {
                        ...
                        redis.on("message", function(channel, msg) {
                            //$$.logger.info(room + 'channel' + '/' + channel + '=====接收到数据' + message);
                            self._emitTextMsg(chat, room, msg);
                        });
                    });
                } catch (err) {
