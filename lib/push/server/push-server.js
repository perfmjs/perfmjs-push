        ...
        startup: function(options) {
            ...
            $$.utils.forEach(this.option('rooms'), function(room, index) {
                ...
                server.of("/" + room).on('connection', function (socket) {
                    //add redis cluster subscribe client
                    var redisSub;
                    try {
                        redisSub = $$.redisCluster.instance.initStartupOptions(self.option('redisClusterNodes'));
                        redisSub.subscribe(self.getPublishKey(room), function(err, reply, redisClient) {
                            if (err) {
                                return;
                            }
                    ...
                    socket.on('disconnect', function () {
                        if (redisSub) {
                            redisSub.unsubscribe(self.option('redisChannelPrefix') + room);
                            redisSub.quit();
                            redisSub.end();
                        }
                    });
                    socket.on('error', function(err) {
                        $$.logger.info("Socket.io error occurred:" + err.message);
                        $$.logger.error(err);
                    });
                    if (self.option('shouldInitDataOnConn')) {
                        self._initDataOnConn(socket, room);
//                        //每次客户端重新连接时接收初始数据(保存到redis里的最后一次成功推送的数据)
//                        redisClient = redis.createClient(options.redisSubPort, options.redisSubHost);
//                        self.option('redisClient', redisClient);
//                        redisClient.get(self.option('redisChannelPrefix') + "lastData/" + room, function(err, reply) {
//                            if (err || !$$.utils.toBoolean(reply)) {
//                                self._initDataOnConn(socket, room);
//                                redisClient.quit();
//                                return;
//                            }
//                            self._emitTextMsg(socket, room, reply);
//                            redisClient.quit();
//                        });
                    }
                });
    ...
    $$.pushServer.defaults = {
        shred: {},
        rooms:['klpk'],
        remoteDataUrls: {'jsbf':'http://xxx.com/yyy'},
