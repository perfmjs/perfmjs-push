...
perfmjs.plugin('pushServer', function($$) {
    $$.base("base.pushServer", {
         ...
         * @param options, e.g. {port:18000,room:'/jsbf'}
         */
        startup: function(options) {
            options = $$.utils.extend({}, options);
            var self = this,
            redis = require('socket.io-redis')({ host: 'ali.no100.com', port: 6379 }),
            server = require('socket.io').listen(options.port, {
                ...
                'allowUpgrades': true
            }).adapter(redis).of(options.room);
            $$.logger.info("Socket.io Server Started with Port: " + options.port  + ", and room: " + options.room);
            ...
            server.on('connection', function (socket) {
                ...
                socket.on("notifyAll", function(jsonData) {
                    ...
                    socket.broadcast.emit("message", (function() {
                        try {
