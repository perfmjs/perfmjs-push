...
perfmjs.plugin('kcPushServer', function($$) {
    $$.base("pushServer.kcPushServer", {
        ...
        _initDataOnConn: function(socket, room) {
            var self = this, startTime =  $$.utils.now();
            var profiler = $$.profiler.newInstance().start(room, ($$.sysConfig.config.get('profile')||{})['tickcount'] || 100, 'tickcount');
            this.option('redisSub').get(this.getStoredLastDataKey(room), function(err, reply) {
                ...
                if (reply) {
                    try {
                        ...
                        profiler.stop(room, $$.utils.now() - startTime, 'kcPushServer#_initDataOnConn代码运行毫秒数过长_redis', 'tickcount');
                        return;
                    }
                    } catch (err) {
                ...
                self.getRemoteData(self.option('remoteDataUrls')[room] + '&time=' + $$.utils.now(), function(responseText) {
                    ...
                    profiler.stop(room, $$.utils.now() - startTime, "kcPushServer#_initDataOnConn代码运行毫秒数过长!");
                });
            });
        },
        /**
